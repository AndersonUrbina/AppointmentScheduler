    ...
    internal static class Program
    {
        ...
        private static void AddAppointmentMenu(AppointmentScheduler scheduler)
        {
            ...
            try
            {
                ...
                var start = PromptDateTime("Enter start time (yyyy-MM-dd HH:mm): ");
                var end = PromptDateTime("Enter end time (yyyy-MM-dd HH:mm): ");
                var room = Prompt("Enter room (e.g., 201A): ");
                var appt = new Appointment(id, patient, provider, start, end, room);
                scheduler.Add(appt);
            }
            catch (DoubleBookingException ex)
            {
                Logger.Warn($"Double booking error: {ex.Message}");
                Console.WriteLine("Error: This appointment conflicts with an existing appointment.");
            }
            catch (InvalidAppointmentTimeException ex)
            {
                Logger.Warn($"Invalid time error: {ex.Message}");
        ...
        private static void RescheduleAppointmentMenu(AppointmentScheduler scheduler)
        {
            ...
            try
            {
                ...
                var newStart = PromptDateTime("New start time (yyyy-MM-dd HH:mm): ");
                var newEnd = PromptDateTime("New end time (yyyy-MM-dd HH:mm): ");
        ...
        private static void ListByProviderMenu(AppointmentScheduler scheduler)
        {
            ...
            if (!list.Any()) { Console.WriteLine($"No appointments found for provider '{provider}'."); return; }
        ...
        private static void ListByDayMenu(AppointmentScheduler scheduler)
        {
            ...
            var day = PromptDateTime("Enter date (yyyy-MM-dd): ").Date;
